...
export async function verifyApiArtifact({
	...
	if (normalizePath(output.entryPoint ?? "") !== "apps/api/src/main.ts") {
		throw new Error(
...
function verifyInputPath(projectRoot: string, inputPath: string): void {
	...
	if (
		normalizedInput.startsWith("apps/api/src/") ||
		normalizedInput.startsWith("packages/db/src/") ||
		normalizedInput.startsWith("packages/shared/src/")
	) {
		return;
	}

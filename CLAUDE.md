...
`docs/` is an Obsidian vault (vault name `docs`). Prefer the `obsidian` CLI for searching and reading rather than ad-hoc `grep`/`Read` — it resolves wikilinks and gives clean output:

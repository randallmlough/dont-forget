...
Native sync package `0.6.0` can leave an offline-written replica unable to push with `unable to checkpoint synced portion of WAL` after reconnect. The app does not maintain a parallel remote-upsert recovery path; native sync failures return through the coordinator's normal failure policy while the locally committed List remains visible. Fixes for native sync reliability belong at the `HouseholdStore`/package boundary, not as duplicate table-specific sync SQL.

...
Native sync package `0.6.0` can leave an offline-written replica unable to push with `unable to checkpoint synced portion of WAL` after reconnect. The app-owned adapter recovers by upserting local `lists`, `items`, and `item_checks` rows to the remote Household DB using the same `updated_at` LWW predicates, then keeps the local List visible. This fallback is intentionally scoped to replicated Household rows and should be removed only after the native sync path can be proven not to strand offline writes.

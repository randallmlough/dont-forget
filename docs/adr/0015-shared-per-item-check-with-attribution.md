...
- **Follow-on resolved (2026-06-30, PowerSync cut-over — [ADR-0018](0018-single-postgres-self-hosted-powersync.md)):** the libSQL app side is gone. `db/schema/household.ts` was deleted and `lib/services/item/item-service.ts` now reads/writes the single-row-per-Item model (synthetic `id` PK, `UNIQUE(item_id)`, `checked_by_user_id`, uncheck-NULLs-columns) against the PowerSync `ProductDatabase` seam. CONTEXT.md and the running app now agree on the shared-per-Item model.

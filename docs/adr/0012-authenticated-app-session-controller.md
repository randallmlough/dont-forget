...
- Sign-out order is centralized in the session sign-out module and exposed by the provider: track `user_signed_out`, reset analytics, dispose the Authenticated App Session controller, clear local synced data via PowerSync `disconnectAndClear`, then call Clerk `signOut()`.

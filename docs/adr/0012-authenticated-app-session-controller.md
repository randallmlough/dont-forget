...
- Sign-out order is centralized in the session sign-out module and exposed by the provider: track `user_signed_out`, reset analytics, dispose the Authenticated App Session controller, clear signed-out session cache/local Household DB files, clear local Current List selections for the signed-out app User, then call Clerk `signOut()`.

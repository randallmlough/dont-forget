...
The controller is Household-wide, not List-singleton-shaped. The Household Session is app-shell state only: active Household, active Member, Members, and Household DB connection metadata. Current List/List/Item data loads separately through the borrowed Current List data source after active Household context exists. Future List switching should replace Current List state/data sources while reusing the active Household resource graph where possible.

## Considered Options

- **Keep route-owned resource composition.** Rejected because Home is only the first signed-in route while the app is under development, and tying Household resource ownership to that screen encourages future screens to duplicate DB lifecycle policy.

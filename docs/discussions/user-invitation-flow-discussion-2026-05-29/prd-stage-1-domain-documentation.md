...
2. As an implementation agent, I want Invitation documentation to stay single-use and token-based, so that I do not add short reusable codes to Invitations.
...
- Add Household Join Code as a domain term for a reusable Household-scoped code/link that lets an authenticated User become a Member of a Household.

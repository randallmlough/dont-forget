...
export async function createTestDirectoryDb(): Promise<TestDirectoryDb> {
  const testClient = await createMigratedTestClient("directory.db", DIRECTORY_MIGRATIONS);
  return {
    ...testClient,
...
export async function createTestHouseholdDb(): Promise<TestHouseholdDb> {
  const testClient = await createMigratedTestClient("household.db", HOUSEHOLD_MIGRATIONS);
  return {
    ...testClient,

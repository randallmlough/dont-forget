...
describe("offline bootstrap cache", () => {
  it("stores cached bootstrap metadata without Household DB auth tokens", async () => {
    ...
    expect(Object.prototype.hasOwnProperty.call(metadata.householdDatabase, "authToken")).toBe(false);
  });
  ...
  it("discards cached Household metadata and local data when fresh bootstrap authorizes a different Household", async () => {
    ...
    const oldBootstrap = bootstrapFixture({ householdId: "hh_old", householdName: "Old" });
    const freshBootstrap = bootstrapFixture({ householdId: "hh_new", householdName: "New" });

    await saveCachedBootstrapMetadata(oldBootstrap, { storage });

    await expect(
      discardCachedBootstrapMetadataIfUnauthorized(freshBootstrap, { storage, deleteHouseholdData }),
    ).resolves.toMatchObject({ activeHousehold: { id: "hh_old" } });
    expect(deleteHouseholdData).toHaveBeenCalledWith("hh_old");
    await expect(storage.getItem(OFFLINE_BOOTSTRAP_CACHE_KEY)).resolves.toBeNull();
  });
});
...
function bootstrapFixture(
  ...
  return {
    ...
    activeHousehold: { id: householdId, name: overrides.householdName ?? "Avery" },
    activeMember: {
      id: "mbr_avery",
      userId: "usr_avery",
      role: "owner",

...
describe("openHouseholdDb", () => {
  ...
  beforeEach(() => {
    ...
    runtime = {
      Database: class extends MockTursoDatabase {
        constructor(options: ConstructorParameters<TursoHouseholdDbRuntime["Database"]>[0]) {
          super(options);
          instances.push(this);
        }
      },
    ...
    fileSystem = { deleteFilesWithPrefix: jest.fn<Promise<void>, [string]>(async (_path) => undefined) };
  });
  ...
  it("adapts query and write results into the app SQL result shape", async () => {
    ...
    await expect(
      db.execute({ sql: "SELECT id, position FROM items WHERE list_id = ?", args: ["lst_groceries"] }),
    ).resolves.toEqual({
      rows: [{ id: "itm_milk", position: 1 }],
      rowsAffected: 0,
      lastInsertRowId: null,
    });
    expect(nativeDb.all).toHaveBeenCalledWith("SELECT id, position FROM items WHERE list_id = ?", [
      "lst_groceries",
    ]);

    await expect(
      db.execute({ sql: "INSERT INTO items (id, name) VALUES (?, ?)", args: ["itm_eggs", "Eggs"] }),
    ).resolves.toEqual({ rows: [], rowsAffected: 2, lastInsertRowId: 42 });
    expect(nativeDb.run).toHaveBeenCalledWith("INSERT INTO items (id, name) VALUES (?, ?)", [
      "itm_eggs",
      "Eggs",
    ]);
  });
...
class MockTursoDatabase {
  ...
  constructor(public readonly options: ConstructorParameters<TursoHouseholdDbRuntime["Database"]>[0]) {}
}

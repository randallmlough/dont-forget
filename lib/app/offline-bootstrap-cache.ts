...
export type CachedBootstrapMetadata = Omit<BootstrapResponse, "householdDatabase"> & {
  householdDatabase: {
    url: string;
    expiresAt: number;
  };
...
export type OfflineBootstrapStorage = Pick<typeof AsyncStorage, "getItem" | "setItem" | "removeItem">;
...
export async function saveCachedBootstrapMetadata(
  ...
  const metadata = cachedMetadataFromBootstrap(bootstrap, options.now ?? Date.now);
  await storageFrom(options).setItem(OFFLINE_BOOTSTRAP_CACHE_KEY, JSON.stringify(metadata));
...
export async function readCachedBootstrapMetadata(
  ...
  try {
    return stripHouseholdDatabaseToken(JSON.parse(raw) as CachedBootstrapMetadata);
  } catch {
    return null;
  }
}

export async function clearCachedBootstrapMetadata(options: CacheOptions = {}): Promise<void> {
...
export async function discardCachedBootstrapMetadataIfUnauthorized(
  ...
  if (!cached || cachedBootstrapIsStillAuthorized(cached, freshBootstrap)) return null;
...
export async function clearCachedHouseholdSession(options: CleanupCacheOptions = {}): Promise<void> {
  const cached = await readCachedBootstrapMetadata(options);
  if (cached) {
    await deleteHouseholdDataFrom(options)(cached.activeHousehold.id);
  }
...
function cachedBootstrapIsStillAuthorized(
  ...
  return cached.user.id === freshBootstrap.user.id && cached.activeHousehold.id === freshBootstrap.activeHousehold.id;
}
...
function stripHouseholdDatabaseToken(metadata: CachedBootstrapMetadata): CachedBootstrapMetadata {
  return {
    user: metadata.user,
...
function deleteHouseholdDataFrom(options: CleanupCacheOptions): (householdId: string) => Promise<void> {

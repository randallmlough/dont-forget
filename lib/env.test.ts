...
describe("environment config", () => {
  ...
  it("requires Clerk development publishable keys outside production", () => {
    expect(() => validateClerkKeyForEnv("staging", "EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY", "pk_live_123"))
      .toThrow("pk_test_");
  });

  it("requires Clerk production publishable keys in production", () => {
    expect(() =>
      validateClerkKeyForEnv("production", "EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY", "pk_test_123"),
    ).toThrow("pk_live_");
  });
  ...
  it("requires confirmation for production operations", () => {
    expect(() => assertProductionConfirmation("production", {})).toThrow("CONFIRM_APP_ENV=production");
    expect(() => assertProductionConfirmation("production", { CONFIRM_APP_ENV: "production" })).not.toThrow();
  });
});

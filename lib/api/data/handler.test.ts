...
type FakeTransactionConfig = {
	households?: Record<string, string[]>; // op.id -> household ids
...
type RecordedCall =
	...
	| { kind: "tombstone"; table: string; id: string }
...
function fakeTransaction(config: FakeTransactionConfig) {
	...
	const tx: DataTransaction = {
		...
		async tombstone(table, id) {
			calls.push({ kind: "tombstone", table, id });
		},
		async uncheckItemCheck(id) {
			calls.push({ kind: "uncheck", id });
		},
	};
	return { tx, calls };
}
...
describe("/api/data handler", () => {
	...
	it("unchecks an item_check without deleting the row (Decision 9)", async () => {
		...
		const response = await handleDataUpload(
			request([{ op: "DELETE", table: "item_checks", id: "c1", data: {} }]),
			d,
		);
		await expect(readJsonResponse(response)).resolves.toMatchObject({
			status: 200,
		});
		expect(calls).toEqual([{ kind: "uncheck", id: "c1" }]);

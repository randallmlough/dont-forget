...
- **Native product-data client**: the iOS app uses `@powersync/op-sqlite` for the local synced database, exposed to services through the app-owned `ProductDatabase` seam (`apps/mobile/src/lib/product-database.ts`); the server Postgres client (`pg`) is limited to the API, migration, reset, and Node test seams under `apps/api/` and `packages/db/`. _Revised 2026-05-16; local synced-store seam moved to the db layer 2026-06-09; replaced by the PowerSync `ProductDatabase` seam 2026-06-30; see ADR-0018 (supersedes ADR-0009), ADR-0011, and ADR-0014._
...
- **Sign-out data boundary**: normal Sign Out clears cached Authenticated App Session metadata and the outgoing User's Current List selection, then disconnects PowerSync without clearing local product rows, queued writes, or the durable database owner. `disconnectAndClear()` is reserved for explicit confirmation after a different User is blocked from the retained database; Sign In as Previous User preserves the previous User's persisted session and Current List selection. _Decided 2026-05-16; local data clearing moved to confirmed different-User recovery 2026-08-17 (see ADR-0020)._
...
- **ORM & migrations**: Drizzle schema and migrations live in `packages/db`; drizzle-kit generates migrations against the single Postgres database; the mobile client runs no migrations (PowerSync client tables are declarative views). _Decided 2026-04-29; single-Postgres migrations adopted 2026-06-30 (see ADR-0018, supersedes ADR-0003)._

...
- **Native Household DB client**: the iOS app uses `@tursodatabase/sync-react-native` behind the app-owned `HouseholdStore` wrapper for local synced Household data access; `@libsql/client` remains limited to server, migration, reset, and Node test seams under `db/server/`. `HouseholdStore` is owned by the db layer at `db/household-store.ts`, not the service layer. _Revised 2026-05-16; HouseholdStore naming applied 2026-05-19; moved to the db layer 2026-06-09; see ADR-0009, ADR-0011, and ADR-0014._
...
- **First-run flow**: on first sign-in, server auto-creates a Household with an initial name randomly selected from the app-owned generated Household name list. The new Household starts with one empty List named "Groceries"; its first Owner can rename it or invite Members later. Invitation accept URLs route through a separate accept flow. _Decided 2026-04-29; List name clarified 2026-05-13; initial Household name generation revised 2026-05-30; Owner-only rename clarified 2026-06-14._

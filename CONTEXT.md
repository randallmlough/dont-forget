...
- **Native product-data client**: the iOS app uses `@powersync/op-sqlite` for the local synced database, exposed to services through the app-owned `ProductDatabase` seam (`lib/services/shared/product-database.ts`); the server Postgres client (`pg`) is limited to server, migration, reset, and Node test seams under `db/server/`. _Revised 2026-05-16; local synced-store seam moved to the db layer 2026-06-09; replaced by the PowerSync `ProductDatabase` seam 2026-06-30; see ADR-0018 (supersedes ADR-0009), ADR-0011, and ADR-0014._

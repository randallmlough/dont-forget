...
import { assertProductionConfirmation, readTursoMigrationConfig, type AppEnv } from "@/lib/env";
...
import { households, invitations, memberships, users } from "./schema/directory";
...
export async function householdDatabasesForReset(directory: DirectoryDb): Promise<HouseholdDatabaseForReset[]> {
  return directory
...
export async function resetDirectoryDatabase(directory: DirectoryDb): Promise<void> {
...
export async function resetHouseholdDatabase(household: HouseholdDb): Promise<void> {
...
async function main(): Promise<void> {
  ...
  assertDatabaseResetConfirmation(appEnv, { CONFIRM_DB_RESET: resetConfirmation });
...
async function resetHouseholdDatabases(
  ...
  for (const householdDatabase of householdDatabases) {
    try {
      ...
      console.log(`[households] ${householdDatabase.id} (${householdDatabase.tursoDbName}) done`);
    } catch (error) {
      failures.push({ id: householdDatabase.id, error });
      console.error(`[households] ${householdDatabase.id} (${householdDatabase.tursoDbName}) failed:`, error);
    }
  }

  if (failures.length > 0) {
    throw new Error(`Database reset failed for ${failures.length} Household DB(s)`);
  }
}

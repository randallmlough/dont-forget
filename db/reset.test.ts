import { households, invitations, memberships, users } from "@/db/schema/directory";
...
describe("database reset", () => {
  it("requires an environment-specific reset confirmation", () => {
    expect(() => assertDatabaseResetConfirmation("local", {})).toThrow("CONFIRM_DB_RESET=local");
    expect(() => assertDatabaseResetConfirmation("local", { CONFIRM_DB_RESET: "local" })).not.toThrow();
  });

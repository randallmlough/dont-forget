...
async function runDrizzleGenerate(config: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const child = spawn("pnpm", ["exec", "drizzle-kit", "generate", `--config=${config}`], {
      stdio: "inherit",
    ...
    child.on("close", (code) => {
      ...
      reject(new Error(`drizzle-kit generate failed for ${config} with exit code ${code}`));
    });
  });
}
